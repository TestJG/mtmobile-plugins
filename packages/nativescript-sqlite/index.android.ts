...
export const openOrCreate = (filePath: string): SQLiteDatabase => {
  const db = android.database.sqlite.SQLiteDatabase.openOrCreateDatabase(filePath, null);
  ...
  const transaction = <T = any>(action: (cancel?: () => void) => T) => transactionRaw(db, action);
